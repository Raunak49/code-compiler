...
function App() {
    ...
    const [languageIcon, setLanguageIcon] = useState("./resources/java.png");
        ...
        try {
            ...
            intervalId = setInterval(async () => {
                ...
                const dataRes  = await axios.get(
                    `http://65.2.40.217/status/${data.id}`
                );
                
                if (dataRes.status === 200) {
                    // console.log(dataRes);
                    setJobDetails(dataRes.data);
                    // console.log(jobDetails);
                    const { status: jobStatus, output: jobOutput } = dataRes.data;
                    setStatus(jobStatus);
                    ...
                    setStatus("Error !!! ");
                    console.error(dataRes.data.error);
                    setOutput(dataRes.data.error);
                    clearInterval(intervalId);
                }
            }, 1000);
        } catch ({ response }) {
